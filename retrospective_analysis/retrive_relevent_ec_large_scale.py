import faiss, torch, os, time, hdbscan, umap, joblib, hashlib
...
def _gpt_relevance_check(rule, query, model) -> bool:
    prompt = f"""
You are an expert in clinical-trial eligibility criteria and medical reasoning.

Your task is to determine whether the two eligibility rules discuss broadly similar clinical concepts — for example, similar types of treatments (e.g., "taxane-based chemotherapy" vs "systemic chemotherapy").

Treat terms that refer to the same clinical **category** or intent as similar, even if the exact wording differs.
...
def retrieve_similar_ec_hdbscan(
    model,
    tokenizer,
    user_query: str,
    ...
    for t in top_trials:
        ...
        text = str(t.get("Eligibility Criteria", ""))
        for line in text.splitlines():
            s = line.strip()
            if s:
                ec_rules.append((s, nct, ttl))
    if not ec_rules:
        return []
    ec_texts = [r[0] for r in ec_rules]

    # ---- Load or compute EC embeddings ----
    emb_cache = Path("../results/ec_temporary_embedding.npy")
    if emb_cache.exists():
        ec_emb = np.load(emb_cache)
    else:
        ec_emb = encode_batch(model, tokenizer, ec_texts).astype("float32")
        np.save(emb_cache, ec_emb)

    # ---- Query embedding ----
    q_emb = encode_batch(model, tokenizer, [user_query]).astype("float32")[0]

    # ---- Normalize once (for cosine + PCA stability) ----
    norm = Normalizer(norm="l2")
    ec_emb_norm = norm.fit_transform(ec_emb)     # (N, D)
    q_emb_norm  = norm.transform(q_emb[None, :]) # (1, D)

    # ---- Pre-filter by cosine similarity ----
    sims_all = cosine_similarity(ec_emb_norm, q_emb_norm).ravel()
    keep_mask = sims_all >= similarity_threshold
    if not np.any(keep_mask):
        return []  # nothing clears the threshold

    ec_emb_norm = ec_emb_norm[keep_mask]
    ec_rules = [r for r, keep in zip(ec_rules, keep_mask) if keep]

    # ---- PCA: fit on EC ONLY; transform query with the same PCA ----
    N, D = ec_emb_norm.shape
    ncomp = int(min(pca_components, N, D))
    pca_path = Path(f"../results/pca_ec_{ncomp}.joblib")

    if pca_path.exists():
        pca = joblib.load(pca_path)
    else:
        pca = PCA(n_components=ncomp, random_state=0).fit(ec_emb_norm)
        joblib.dump(pca, pca_path)

    ec_pca = pca.transform(ec_emb_norm)         # (N', ncomp)
    q_pca  = pca.transform(q_emb_norm)          # (1, ncomp)

    # ---- UMAP: fit on EC ONLY; transform query with the same reducer ----
    umap_embed_path = Path(f"../results/ec_umap_pca{ncomp}_umap{umap_components}.npy")
    umap_model_path = Path(f"../results/umap_reducer_pca{ncomp}_umap{umap_components}.joblib")

    if umap_embed_path.exists() and umap_model_path.exists():
        ec_umap = np.load(umap_embed_path)
        ...
        ec_umap = reducer.embedding_
        np.save(umap_embed_path, ec_umap)
        joblib.dump(reducer, umap_model_path)

    q_umap = reducer.transform(q_pca)  # (1, umap_components)

    # ---- Cluster EC UMAP; label query via approximate_predict ----
    ...
    labels_ec = clusterer.labels_
    label_q, _ = hdbscan.approximate_predict(clusterer, q_umap)
    label = int(label_q[0])

    member_idx = np.where(labels_ec == label)[0]
    if member_idx.size == 0:
        return []
    # rank those members by cosine similarity using normalized embeddings
    # sims = cosine_similarity(ec_emb_norm[member_idx], q_emb_norm).ravel()
    # order = np.argsort(-sims)
    # member_idx = member_idx[order]
    # sims = sims[order]
    # if top_k is not None:
    #     member_idx = member_idx[:top_k]
    #     sims = sims[:top_k]
...
def retrieve_similar_ec(model, tokenizer, nli_model, nli_tokenizer, user_query, top_trials, threshold, evaluation = None):
    ...
    for nct, (idx, score) in best_by_nct.items():
        ...
        ok = True
        if evaluation == "llm":
            # Only check the per-trial winner to save tokens/time
...
def main():
    ...
    for application in applications:
        trial_csv = f"../results/retrieved_trials_{application.replace("_", " ")}.csv"
        ...
        for trial_id in inclusion_trial_data["Trial_ID"]:
            ...
            for idx, criterion in enumerate(inclusion_criteria + exclusion_criteria):
                ...
                # top_ec = retrieve_similar_ec_hdbscan(
                #     model=model,
                #     tokenizer=tokenizer,
                #     user_query=criterion,
                #     top_trials=top_trials,
    ...
    out_path = Path("../results/full_rule_summary_nli_check_0.6.json")
